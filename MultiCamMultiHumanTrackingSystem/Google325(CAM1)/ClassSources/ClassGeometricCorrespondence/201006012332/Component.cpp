...
//set block locatons
bool BodyInfo::LocateBlocks(const CameraLine3D * headline, CameraLine3D & groundline, bool h_is_got, double h )
{
	groundline = *headline;
	...
	for (int k=0; k<NUM_OF_BLOCKS; k++)
	{
		bodyPt[k] = cvPoint(headline->imageLoc.x, groundline.imageLoc.y - block_height*((double)k+1.0));
...
{
	bool result = LocateBlocks(headline, groundline, h_is_got, h);
	float max_val;
...
double AppearanceComparison(const BodyInfo* ob1, const BodyInfo* ob2, int & matchCnt, OBJ_CONFIDENCE & level, double & best_color_score, int & best_block_index)
{
...
ObjectInfo createObjectInfo(BodyInfo* biptr, int obsIndex_cam1, int obsIndex_cam2, OBJ_CONFIDENCE obj_conf/* = OBJ_NON_DEFINED*/)
{
...
//return: RETURN_NO_ERROR, RETURN_CONTRADICT, RETURN_ALREADY_DONE
//output: confMap
//input: cam1Alive, cam2Alive(already updated)(they must be updated every time index from all local tracking systems)
RETURN_STATUS observationIntegrate(OBJ_CONFIDENCE** confMap, int obsIndex_cam1, int obsIndex_cam2, bool* cam1Alive, bool* cam2Alive)
{
	if (confMap[obsIndex_cam1][obsIndex_cam2] == OBJ_HIGH_CONF) return RETURN_ALREADY_DONE;
	if (confMap[obsIndex_cam1][obsIndex_cam2] == OBJ_HIGH_BLOCK) return RETURN_CONTRADICT;
	if (!(cam1Alive[obsIndex_cam1] && cam2Alive[obsIndex_cam2])) return RETURN_CONTRADICT;
	
	for (int t=0; t<MAX_TARGET_NUM; t++)
	{
		if (cam2Alive[t]) confMap[obsIndex_cam1][t] = OBJ_HIGH_BLOCK;
		if (cam1Alive[t]) confMap[t][obsIndex_cam2] = OBJ_HIGH_BLOCK;
	}
	confMap[obsIndex_cam1][obsIndex_cam2] = OBJ_HIGH_CONF;

	return RETURN_NO_ERROR;
}
RETURN_STATUS observationDisappear(OBJ_CONFIDENCE** confMap, int cam_id, int obsIndex_camX, bool* cam1Alive, bool* cam2Alive)
{
	if (cam_id == CAM1) 
	{
		...
		for (int t=0; t<MAX_TARGET_NUM; t++)
		{
			confMap[obsIndex_camX][t] = OBJ_NON_DEFINED;
		}
		return RETURN_NO_ERROR;
	}
	else if (cam_id == CAM2)
	{
		...
		for (int t=0; t<MAX_TARGET_NUM; t++)
		{
			confMap[t][obsIndex_camX] = OBJ_NON_DEFINED;
		}
		return RETURN_NO_ERROR;
	}
	else return RETURN_CONTRADICT;
}
RETURN_STATUS observationShowIndep(OBJ_CONFIDENCE** confMap, int cam_id, int obsIndex_camX, bool* cam1Alive, bool* cam2Alive)
{
	bool valid = true;
	if (cam_id == CAM1)
	{
		for (int t=0; t<MAX_TARGET_NUM; t++)
		{
			valid &= (confMap[obsIndex_camX][t] == OBJ_NON_DEFINED);
		}
		if (!valid) 
		...
		for (int t=0; t<MAX_TARGET_NUM; t++)
		{
			...
			{
				if (confMap[j][t] == OBJ_HIGH_CONF) 
					highConfCnt++;
			}

			if (highConfCnt == 1) //normal case of high conf integration
			{
				confMap[obsIndex_camX][t] = OBJ_HIGH_BLOCK;
			}
			else if (highConfCnt > 1) 
				return RETURN_CONTRADICT;
			else if (highConfCnt == 0)
			{
				//Nice, do nothing
				confMap[obsIndex_camX][t] = OBJ_NON_DEFINED;
			}
	...
	else if (cam_id == CAM2)
	{
		for (int t=0; t<MAX_TARGET_NUM; t++)
		{
			valid &= (confMap[t][obsIndex_camX] == OBJ_NON_DEFINED);
		}
		if (!valid) 
			return RETURN_CONTRADICT;
		...
		for (int t=0; t<MAX_TARGET_NUM; t++)
		{
			...
			for (int j=0; j<MAX_TARGET_NUM; j++)
			{
				if (confMap[t][j] == OBJ_HIGH_CONF) 
					highConfCnt++;
			}

			if (highConfCnt == 1) //normal case of high conf integration
			{
				confMap[t][obsIndex_camX] = OBJ_HIGH_BLOCK;
			}
			else if (highConfCnt > 1) 
				return RETURN_CONTRADICT; //error case
			else if (highConfCnt == 0) //normal case of no high conf integration
			{
				//Nice, do nothing
				confMap[t][obsIndex_camX] = OBJ_NON_DEFINED;
			}
...
void printConfMapOnConsole(OBJ_CONFIDENCE** confMap, bool* cam1Alive, bool* cam2Alive)
{
...
void findAllPairMode(int t, Rule rl, List* lt, double** scoreMap, OBJ_CONFIDENCE** confMap, int* pairNumCtr, double* bestScore, vector<List*>* pairModeList)
{
	...
	for (int j=0; j<MAX_TARGET_NUM; j++)
	{
		if (confMap[t][j] == OBJ_NON_DEFINED || !(rl.checkValidPairFromRule(t, j))) continue;
